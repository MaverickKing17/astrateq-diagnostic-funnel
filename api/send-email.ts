...
export default async function handler(req: any, res: any) {
  ...
  try {
    const { email, firstName, score, tierName, riskProfile, privacyAlignment, attentionReadiness, tierDesc } = req.body || {};
    ...
    const emailHtml = `
      ...
      <head>
        ...
        <style>
          body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; background-color: #f0f4f8; color: #1e293b; margin: 0; padding: 0; }
          .container { max-width: 600px; margin: 40px auto; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 12px rgba(15, 23, 42, 0.05); border: 1px solid #e2e8f0; }
          .header { background-color: #0b1a30; padding: 32px 24px; text-align: center; border-bottom: 4px solid #0284c7; }
          .logo { color: #38bdf8; font-size: 24px; font-weight: 800; letter-spacing: -0.025em; margin: 0; }
          ...
          .score-box { background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; text-align: center; margin-bottom: 28px; }
          .score-value { font-size: 48px; font-weight: 800; color: #0284c7; line-height: 1; }
          .score-label { font-size: 12px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 0.1em; margin-top: 8px; }
          .metrics-table { width: 100%; border-collapse: collapse; margin-bottom: 28px; }
          .metrics-table td { padding: 12px; border-bottom: 1px solid #f1f5f9; font-size: 14px; }
          .metrics-table td.label { font-weight: 600; color: #475569; width: 45%; }
          .metrics-table td.value { font-weight: 700; color: #0f172a; text-align: right; }
          .badge { display: inline-block; background-color: #e0f2fe; color: #0369a1; padding: 4px 8px; border-radius: 6px; font-size: 12px; font-weight: 700; }
          .explanation-box { background-color: #f0fdf4; border-left: 4px solid #22c55e; padding: 16px; border-radius: 0 8px 8px 0; margin-bottom: 28px; }
          .explanation-title { font-size: 13px; font-weight: 700; color: #15803d; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px; }
          .explanation-text { font-size: 13px; line-height: 1.5; color: #166534; margin: 0; }
          .cta-button { display: block; text-align: center; background-color: #0284c7; color: #ffffff !important; text-decoration: none; padding: 14px 24px; border-radius: 10px; font-weight: 700; font-size: 15px; margin-bottom: 24px; }
          .footer { background-color: #f8fafc; padding: 24px; text-align: center; border-top: 1px solid #f1f5f9; font-size: 12px; color: #64748b; line-height: 1.5; }
